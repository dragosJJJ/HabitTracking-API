import requests
from datetime import datetime

TOKEN = "ajfigaublopaknhj423h6b3n5"
USER = "dragos"
GRAPH_ID = "graph1"

user_endpoint = "https://pixe.la/v1/users"


user_params = {
    "token": TOKEN,
    "username": USER,
    "agreeTermsOfService": "yes",
    "notMinor": "yes",
}

# response = requests.post(url=user_endpoint, json=user_params)
# print(response.text)

graph_endpoint = f"{user_endpoint}/{USER}/graphs"

graph_config = {
    "id": GRAPH_ID,
    "name": "Exercise Graph",
    "unit": "reps",
    "type": "int",
    "color": "ajisai",
}
headers = {
    "X-USER-TOKEN": TOKEN
}
# response = requests.post(url=graph_endpoint, json=graph_config, headers=headers)
# print(response.text)

pixel_creation_endpoint = f"{user_endpoint}/{USER}/graphs/{GRAPH_ID}"

today = datetime.now()

pixel_data = {
    "date": today.strftime("%Y%m%d"),
    "quantity": input("How many push-ups did you do today?"),
    "optionalData":"{\"exercise\":\"push-ups\"}"
}
# response = requests.post(url=pixel_creation_endpoint, json=pixel_data, headers=headers)
# print(response.text)

update_endpoint = f"{user_endpoint}/{USER}/graphs/{GRAPH_ID}/{today.strftime('%Y%m%d')}"
new_pixel_data = {
    "quantity": 100
}

# response = requests.post(url=update_endpoint, json=new_pixel_data, headers=headers)
# print(response.text)

delete_endpoint = f"{user_endpoint}/{USER}/graphs/{GRAPH_ID}/{today.strftime('%Y%m%d')}"
response = requests.delete(url=delete_endpoint, headers=headers)
print(response.text)
